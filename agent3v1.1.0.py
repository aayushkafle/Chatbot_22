...
searchresultlist = chatbot_query(query, size=10)
...
resultlist = pd.DataFrame(searchresultlist, columns=['a'])
print(resultlist)
...
similarity = -1
for i in range(0, 10):
    s = lda_model2.show_topics(10)[i][1]
    print(s)
    #print(''.join(x for x in s if x.isalpha()))

"""{0: {'Dogs are awesome.': 'Response one'}, 1: {'Dolphins are swimming mammals.': 'Response Two'}, 2: {'Cats are beautiful animals.': 'Response Three'}}"""

# preprocessing context history
context = 'drug addiction alcohol sex hospital{0: {\'Dogs are awesome.\': \'Response one\'}, 1: {\'Dolphins are swimming mammals.\': \'Response Two\'}, 2: {\'Cats are beautiful animals.\': \'Response Three\'}}'
context = preprocess(context)
context

# compute jaccard similarity

a = lda_model2.show_topics()
similarities = [[],[]]
...
# return list with jaccard similarities, check if jacsim with query is nonzero somewhere, if yes, compare and 
#  |-> choose the topic with the highest jacsim, if not, compare jacsims of context and take highest, deliver document
  ...
  if(max_sim != 0):
      ...
      for i in range(0, len(similarities[1])):
          if(max_sim - 0.0000005 < similarities[1][i] < max_sim + 0.0000005):
              highest_topic_index = i
  print(highest_topic_index)
  print(similarities[0])
  print(similarities[1])
  return highest_topic_index

index = topic_choice_heuristic()
print(searchresultlist[index])